...
const News = () => {
  ...
  return (
    <div className="bg-white dark:bg-darkBg flex flex-col rounded-lg p-3">
      ...
      <ul className="list-decimal flex flex-col gap-3 mt-4">
        {news.map((n, i) => (
          <li className="flex flex-col w-full " key={n.pubDate}>
            <p className=" text-gray-900 dark:text-gray-100 font-medium text-ellipsis overflow-hidden w-full whitespace-nowrap">
              {n.title}
            </p>
            <div className="text-gray-600 dark:text-gray-400">{`${i} hours ago`}</div>
          </li>
        ))}
      </ul>
    </div>
